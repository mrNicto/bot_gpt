...
def limit(message):
    ...
    datam = cur.fetchone()
    msg = f"Превышен общий лимит GPT {MAX_USER_STT_BLOCKS}. Использовано {data[0]} блоков. Доступно: {MAX_USER_TOKENS_BLOCKS - data[0]}"
    bot.send_message(user_id, msg)
    msg = f"Превышен общий лимит SpeechKit STT {MAX_USER_STT_BLOCKS}. Использовано {datam[0]} блоков. Доступно: {MAX_USER_STT_BLOCKS - datam[0]}"
    bot.send_message(user_id, msg, reply_markup=markup)
    cur.close()
    conn.close()
